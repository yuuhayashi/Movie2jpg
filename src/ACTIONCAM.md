 ...
 * 出力形式はMP4(h265)になってしまう  
    出力されるのはカメラの仕様上MP4(h265) or MOV(3gp4)になります。
...
2. 生成された静止画ファイルの更新日時を撮影時刻に書き換える

3. JPEGファイルの撮影時刻とGPSロガーの時刻情報を突き合わせて JPEGのExif情報に位置情報を書き込む

4. Mapillary-toolsを使って Mapillary サイトにJPEGファイルを一括アップロードする。

と加工します。  
加工の手順は [別のページ](README.md) で紹介します。
